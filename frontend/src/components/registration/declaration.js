...
export const PLAIN_DECLRATIONS = {
  ...
  questions: {
    ...
    ad_5: 'That it shall ensure that all its employees, personnel and sub-contractors comply with the standards of conduct listed in Section 3 of the UN Secretary-General\'s Bulletin on' +
...
const renderQuestions = () => (
  <Grid item>
    {Object.keys(DECLRATIONS.questions).map((key, index) => (
      <DeclarationRow
        message={DECLRATIONS.questions[key]}
        index={index}
        key={index}
      />
    ))}
    ...
    <Grid item xs={12}>
      <Grid container direction="column" spacing={16}>
        <Grid className={classes.headerContainer} item>
          <Typography type="body2">{DECLRATIONS.header}</Typography>
        </Grid>
